...
function toCompetition(row: CompetitionRow & { country?: PlaceRow | null }): CompetitionInterface {
...
export class DrizzleCompetitionRepository implements ICompetitionRepository {
  ...
  async create(data: Partial<CompetitionInterface>): Promise<CompetitionInterface> {
    const [competition] = await this.db
      .insert(competitions)
      .values({ ...(data as typeof competitions.$inferInsert), updatedAt: new Date() })
      .returning();
  ...
  async update(id: string, data: Partial<CompetitionInterface>): Promise<CompetitionInterface | null> {
    const [competition] = await this.db
      .update(competitions)
      .set({ ...(data as Partial<typeof competitions.$inferInsert>), updatedAt: new Date() })
      .where(eq(competitions.id, id))
      .returning();
  ...
  async delete(id: string): Promise<CompetitionInterface> {
    const [competition] = await this.db.delete(competitions).where(eq(competitions.id, id)).returning();
    if (!competition) {
