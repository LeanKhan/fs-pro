...
export class DrizzlePlayerMatchRepository implements IPlayerMatchRepository {
  ...
  async findById(id: string): Promise<PlayerMatchDetailsInterface | null> {
    const row = await this.db.query.playerMatchDetails.findFirst({ where: eq(playerMatchDetails.id, id) });
  ...
  async createMany(data: Partial<PlayerMatchDetailsInterface>[]): Promise<PlayerMatchDetailsInterface[]> {
    ...
    const rows = await this.db
      ...
      .values(data.map((d) => ({ ...(d as typeof playerMatchDetails.$inferInsert), updatedAt: new Date() })))
      .returning();
  ...
  async update(id: string, data: Partial<PlayerMatchDetailsInterface>): Promise<PlayerMatchDetailsInterface | null> {
    const [row] = await this.db
      ...
      .set({ ...(data as Partial<typeof playerMatchDetails.$inferInsert>), updatedAt: new Date() })
  ...
  async delete(id: string): Promise<PlayerMatchDetailsInterface> {
    const [row] = await this.db.delete(playerMatchDetails).where(eq(playerMatchDetails.id, id)).returning();
    if (!row) {
