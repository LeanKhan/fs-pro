// Match-tick-only position/block-lookup helpers (getATTMID, getGK,
// findFreeBlock, sortFromKeeperDown, etc) moved to
// simulation/utils/players.ts - see the simulation-engine-isolation plan.
// Everything below is CRUD/training-layer only (rating/value/wage/growth),
// used by player.service.ts/player.controller.ts/player-training.service.ts/
// player-lifecycle.service.ts/the migration scripts - never called during
// live match simulation.
import { ratingFactors, postitionFactors, ageFactors } from './player-factors';
import { randomBetween, pickRandomFromArray } from '../helpers/misc';
import { PlayerInterface, IPlayerAttributes, Multipliers, AllMultipliers } from '../interfaces/Player';
